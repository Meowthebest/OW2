...
await wait();
clickSelector(app.document, '.role-choice-grid .role-tank', 'global Tank role option');
await wait();
assert.equal(app.document.querySelector('button[aria-label="Toggle Tank for Alpha"]')?.disabled, false, 'A player role should remain selectable when it is not yet globally included.');
clickSelector(app.document, 'button[aria-label="Toggle Tank for Alpha"]', 'Alpha Tank role toggle');
await wait();
assert.equal(app.document.querySelector('.role-choice-grid .role-tank')?.getAttribute('aria-pressed'), 'true', 'Selecting Tank for a player should include Tank globally.');
clickSelector(app.document, 'button[aria-label="Toggle Tank for Alpha"]', 'Alpha Tank role toggle');
await wait();
clickSelector(app.document, '.party-player-config:first-child .player-role-flex', 'Alpha Flex role option');
await wait();
assert.ok(['Tank', 'Damage', 'Support'].every((role) => app.document.querySelector('button[aria-label="Toggle ' + role + ' for Alpha"]')?.getAttribute('aria-pressed') === 'true'), 'Flex should always select all three roles.');
clickSelector(app.document, 'button[aria-label="Toggle Tank for Alpha"]', 'Alpha Tank role toggle');
await wait();
...
assert.match(reusableWinner.getAttribute('aria-label') ?? '', /\. 1 win/, 'The winner card should show its per-hero win count.');
